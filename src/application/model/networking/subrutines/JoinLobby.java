 ...
 * l'host della partita si è sconnesso prima dell'inizio di questa e di
...
public class JoinLobby implements Runnable {
	 ...
	 * che fa da host alla lobby. In caso di insuccesso ritorna al menù; in caso di
	 * successo rimane in attesa del messaggio di inizio della partita. Se l'host si
	 * disconnette durante l'attesa il metodo ricarica la pagina del menù.
	 */
	@Override
	public void run() {
		Socket host = null;
		...
		if (host != null) {
			...
			if (!crashed) {
				Platform.runLater(new LoadTable(tc, true));
			} else {
				Platform.runLater(new LoadScene("MultiPlayerHub.fxml"));
	...
	/**
	 * Ritorna se il socket è stato disconnesso
	 * 
	 * @return true se il socket è disconnesso
	 */
