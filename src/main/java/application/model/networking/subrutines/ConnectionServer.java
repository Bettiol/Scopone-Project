...
public class ConnectionServer implements Runnable {
		...
		if (!killed) {
			...
			for (int i = 0; i < connections.length; i++) {
				try {
					...
					connections[i] = null;
					System.out.println("Qualcuno si � disconnesso");
					// e.printStackTrace();
				}
			}

			//TODO Platform.runLater(new LoadTable(connections));
	...
	/**
	 * Specifica se il ServerSocket � chiuso o no
	 * 
	 * @return true se il ServerSocket � chiuso
	 */
